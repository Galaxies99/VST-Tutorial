...
Module Info.
  ...
  Definition source_file := "add.c"%string.
...
Definition _add : ident := 56%positive.
Definition _main : ident := 58%positive.
Definition _slow_add : ident := 57%positive.
Definition _uadd : ident := 55%positive.
...
Definition f_uadd := {|
  ...
  fn_vars := nil;
  fn_temps := ((_z, tuint) :: nil);
  fn_body :=
(Ssequence
  (Sset _z (Ebinop Oadd (Etempvar _x tuint) (Etempvar _y tuint) tuint))
  (Sreturn (Some (Etempvar _z tuint))))
|}.
...
((___builtin_bswap,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=true; cc_unproto:=false; cc_structret:=false|})) ::
 (_uadd, Gfun(Internal f_uadd)) :: (_add, Gfun(Internal f_add)) ::
 (_slow_add, Gfun(Internal f_slow_add)) :: nil).

Definition public_idents : list ident :=
(_slow_add :: _add :: _uadd :: ___builtin_debug :: ___builtin_nop ::
