...
Module Info.
  ...
  Definition source_file := "fact.c"%string.
...
Definition _fact : ident := 54%positive.
Definition _fact_loop : ident := 57%positive.
...
(Sifthenelse (Ebinop Oeq (Etempvar _n tuint) (Econst_int (Int.repr 0) tint)
  ...
  (Ssequence
    (Scall (Some _t'1)
      (Evar _fact (Tfunction (Tcons tuint Tnil) tuint cc_default))
      ((Ebinop Osub (Etempvar _n tuint) (Econst_int (Int.repr 1) tint) tuint) ::
       nil))
    (Sreturn (Some (Ebinop Omul (Etempvar _n tuint) (Etempvar _t'1 tuint)
                     tuint)))))
|}.

Definition f_fact_loop := {|
  fn_return := tuint;
  fn_callconv := cc_default;
  fn_params := ((_n, tuint) :: nil);
  fn_vars := nil;
  fn_temps := ((_i, tuint) :: (_r, tuint) :: nil);
  fn_body :=
(Ssequence
          ...
          (Sset _r
            (Ebinop Omul
              (Ebinop Oadd (Etempvar _i tuint) (Econst_int (Int.repr 1) tint)
                tuint) (Etempvar _r tuint) tuint)))
        (Sset _i
          (Ebinop Oadd (Etempvar _i tuint) (Econst_int (Int.repr 1) tint)
            tuint))))
    (Sreturn (Some (Etempvar _r tuint)))))
|}.
...
((___builtin_ais_annot,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=true; cc_unproto:=false; cc_structret:=false|})) ::
 (_fact, Gfun(Internal f_fact)) ::
 (_fact_loop, Gfun(Internal f_fact_loop)) :: nil).

Definition public_idents : list ident :=
(_fact_loop :: _fact :: ___builtin_debug :: ___builtin_nop ::
