...
Definition _append : ident := 64%positive.
Definition _append2 : ident := 65%positive.
...
Definition f_append2 := {|
  fn_return := tvoid;
  fn_callconv := cc_default;
  fn_params := ((_x, (tptr (tptr (Tstruct _list noattr)))) ::
                (_y, (tptr (Tstruct _list noattr))) :: nil);
  fn_vars := nil;
  fn_temps := ((_h, (tptr (Tstruct _list noattr))) :: nil);
  fn_body :=
(Ssequence
  (Sset _h
  ...
  (Ssequence
    (Swhile
      (Ebinop One (Etempvar _h (tptr (Tstruct _list noattr)))
        (Ecast (Econst_int (Int.repr 0) tint) (tptr tvoid)) tint)
      (Ssequence
        (Sset _x
          (Eaddrof
            (Efield
              (Ederef (Etempvar _h (tptr (Tstruct _list noattr)))
                (Tstruct _list noattr)) _tail (tptr (Tstruct _list noattr)))
...
(_append2 :: _append :: _sumlist :: ___builtin_debug :: ___builtin_nop ::
