...
Module Info.
  ...
  Definition source_file := "gcd.c"%string.
...
Definition _gcd : ident := 56%positive.
...
Definition f_gcd := {|
  ...
  fn_params := ((_n, tint) :: (_m, tint) :: nil);
  fn_vars := nil;
  fn_temps := ((_r, tint) :: (_t'1, tint) :: nil);
  fn_body :=
(Ssequence
  (Sset _r (Ebinop Omod (Etempvar _m tint) (Etempvar _n tint) tint))
  (Sifthenelse (Ebinop Oeq (Etempvar _r tint) (Econst_int (Int.repr 0) tint)
                 tint)
    (Sreturn (Some (Etempvar _n tint)))
    (Ssequence
      (Scall (Some _t'1)
        (Evar _gcd (Tfunction (Tcons tint (Tcons tint Tnil)) tint cc_default))
        ((Etempvar _r tint) :: (Etempvar _n tint) :: nil))
...
((___builtin_ais_annot,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=true; cc_unproto:=false; cc_structret:=false|})) ::
 (_gcd, Gfun(Internal f_gcd)) :: nil).

Definition public_idents : list ident :=
(_gcd :: ___builtin_debug :: ___builtin_nop :: ___builtin_write32_reversed ::
