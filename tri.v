...
Module Info.
  ...
  Definition source_file := "tri.c"%string.
...
Definition _tri_for : ident := 56%positive.
Definition _tri_while : ident := 57%positive.

Definition f_tri_for := {|
  fn_return := tuint;
  fn_callconv := cc_default;
  fn_params := ((_n, tint) :: nil);
  fn_vars := nil;
  fn_temps := ((_s, tuint) :: (_i, tint) :: nil);
  fn_body :=
(Ssequence
      ...
      (Sloop
        (Ssequence
          (Sifthenelse (Ebinop Olt (Etempvar _i tint) (Etempvar _n tint)
            ...
            Sbreak)
          (Sset _s
            (Ebinop Oadd (Etempvar _s tuint) (Etempvar _i tint) tuint)))
        (Sset _i
...
Definition f_tri_while := {|
  fn_return := tuint;
  fn_callconv := cc_default;
  fn_params := ((_n, tint) :: nil);
  fn_vars := nil;
  fn_temps := ((_s, tuint) :: nil);
  fn_body :=
(Ssequence
  (Sset _s (Econst_int (Int.repr 0) tint))
  (Ssequence
      ...
      (Ssequence
        (Sset _n
          (Ebinop Osub (Etempvar _n tint) (Econst_int (Int.repr 1) tint)
            tint))
        (Sset _s (Ebinop Oadd (Etempvar _s tuint) (Etempvar _n tint) tuint))))
    (Sreturn (Some (Etempvar _s tuint)))))
|}.
...
((___builtin_ais_annot,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=true; cc_unproto:=false; cc_structret:=false|})) ::
 (_tri_for, Gfun(Internal f_tri_for)) ::
 (_tri_while, Gfun(Internal f_tri_while)) :: nil).

Definition public_idents : list ident :=
(_tri_while :: _tri_for :: ___builtin_debug :: ___builtin_nop ::
